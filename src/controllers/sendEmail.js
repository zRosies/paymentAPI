...
async function sendEmailMessage(req, productId, retries = 3) {
  ...
  try {
    ...
    // -------------------- Recursion here ----------------
    // Trying to resend the email 3 after 5s if there is an error.
    if (response.error) {
      if (retries > 0) {
        console.log(`Email failed, retrying: ${retries}`);
        setTimeout(() => {
          sendEmailMessage(req, productId, retries - 1);
        }, 5000);
      }
    }
